    ...
    public TileCoords moveTo(TileCoords chosenPos) {
        ...
        if (chosenPos.x == playerPos.x) {
            if (playerPos.y < chosenPos.y) {
                while (playerPos.y != chosenPos.y && !(maze.horizontalWalls[playerPos.y])) {
                    playerPos.y++;
                }
            }
            else if (playerPos.y > chosenPos.y) {
                while (playerPos.y != chosenPos.y && !(maze.horizontalWalls[playerPos.y-1])) {
                    playerPos.y--;
                }
            }
        }
            ...
            if (playerPos.x < chosenPos.x) {
                while (playerPos.x != chosenPos.x && !(maze.horizontalWalls[playerPos.x])) {
                    playerPos.x++;
                }
            }
            else if (playerPos.x > chosenPos.x) {
                while (playerPos.x != chosenPos.x && !(maze.horizontalWalls[playerPos.x-1])) {
                    playerPos.x--;
                }
            }
        }
